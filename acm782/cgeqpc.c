/* cgeqpc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int cgeqpc(integer *job, integer *m, integer *n, integer *k,
	 complex *a, integer *lda, complex *c__, integer *ldc, integer *dsrd,
	integer *offset, real *ircond, integer *lacptd, integer *jpvt,
	complex *tau, complex *x, real *svlues, real *mxnm, complex *work,
	integer *lwork, real *rwork)
{
    ...
    static complex sine;
    static real temp, smin, smax, temp2;
...
/*     work store the exact column norms. */
	...
	i__2 = *m - *offset;
	rwork[i__] = scnrm2(&i__2, &a[*offset + 1 + i__ * a_dim1], &c__1);
...
/*        Generate elementary reflector H(i). */
	...
	if (i__ < *m) {
	    i__2 = *m - i__ + 1;
	    clarfg(&i__2, &a[i__ + i__ * a_dim1], &a[i__ + 1 + i__ * a_dim1],
		     &c__1, &tau[i__]);
	} else {
	    clarfg(&c__1, &a[*m + *m * a_dim1], &a[*m + *m * a_dim1], &c__1,
		    &tau[*m]);
	}
...
/*           Apply H(I) to C(I:M,1:K) from the left. */
	    ...
	    i__2 = *m - i__ + 1;
	    r_cnjg(&q__1, &tau[i__]);
	    clarf("Left", &i__2, k, &a[i__ + i__ * a_dim1], &c__1, &q__1, &
...
/*           Apply the transpose of H(I) to C(1:K,I:M) from the right. */
	    ...
	    i__2 = *m - i__ + 1;
	    clarf("Right", k, &i__2, &a[i__ + i__ * a_dim1], &c__1, &tau[i__]
...
/*        Check new column for independence. */
	...
	if (i__ == 1) {
		...
		*lacptd = 1;
	    ...
	    smaxpr = slasmx(&i__) * *mxnm;
	    r__1 = smaxpr * *ircond;
	    if (clauc1(&i__, &x[1], &smin, &a[i__ * a_dim1 + 1], &a[i__ +
		    i__ * a_dim1], &r__1)) {
...
/*           Factor remaining columns. */
	    ...
	    i__1 = *m - i__;
	    i__2 = *n - i__;
	    cgeqrf(&i__1, &i__2, &a[i__ + 1 + (i__ + 1) * a_dim1], lda, &tau[
...
/*              Apply them to C(I+1:M,1:K) from the left. */
		...
		i__1 = *m - i__;
		i__2 = mn - i__;
		cunmqr("Left", "Conjugate Transpose", &i__1, k, &i__2, &a[
...
/*              right. */
		...
		i__1 = *m - i__;
		i__2 = mn - i__;
		cunmqr("Right", "No Transpose", k, &i__1, &i__2, &a[i__ + 1
...
/* Computing MAX */
	...
	for (i__ = max(i__1,i__2); i__ <= i__3; ++i__) {
	    i__1 = i__ - 1;
	    claic1(&c__2, &i__1, &x[1], &smin, &a[i__ * a_dim1 + 1], &a[i__
