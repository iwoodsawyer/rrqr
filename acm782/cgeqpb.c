/* cgeqpb.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int cgeqpb(integer *job, integer *m, integer *n, integer *k,
	 complex *a, integer *lda, complex *c__, integer *ldc, integer *jpvt,
	real *ircond, real *orcond, integer *rank, real *svlues, complex *
...
/*        Compute the optimal complex workspace. */
	...
	if (*job == 1) {
	    nb = ilaenv(&c__1, "CGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)6,
		    (ftnlen)1);
/* Computing MAX */
	    i__1 = *n * 3, i__2 = *n * nb;
...
/* Computing MAX */
	    i__1 = ilaenv(&c__1, "CGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)
		    6, (ftnlen)1), i__2 = ilaenv(&c__1, "CUNMQR", " ", m, n,
...
/* Computing 2nd power */
	    i__1 = max(*k,*n);
	    itemp = (integer) sqrt((real) (i__1 * i__1 + (*lwork << 2) - (mn
		    << 3)));
	    nb = (itemp - max(*k,*n)) / 2;
	}
    }

    block = nb > 1 && nb >= ilaenv(&c__2, "CGEQRF", " ", m, n, &c__0, &c__0,
	    (ftnlen)6, (ftnlen)1) && mn >= ilaenv(&c__3, "CGEQRF", " ", m, n,
	     &c__0, &c__0, (ftnlen)6, (ftnlen)1);
...
/*     *************************************************** */
    ...
    i__1 = *lwork - (mn << 1);
    cgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &c__1, &
...
/* Computing MIN */
	...
	i__1 = kk - 1;
	cgeqpw(m, &lwsize, &kb, &i__1, &lacptd, &a[a_offset], lda, &jpvt[1],
...
/*              Accumulate Householder vectors in a block reflector. */
	    ...
	    i__1 = *m - kk + 1;
	    clarft("Forward", "Columnwise", &i__1, &lacptd, &a[kk + kk *
...
/*                 from the right. */
		...
		i__1 = *m - kk + 1;
		clarfb("Right", "No Transpose", "Forward", "Columnwise", k, &
...
/* Computing MIN */
		...
		for (i__ = kk + lacptd; i__ <= i__1; ++i__) {
		    --mvidx;
		    cswap(m, &a[i__ * a_dim1 + 1], &c__1, &a[mvidx * a_dim1
			    + 1], &c__1);
		    itemp = jpvt[i__];
		    jpvt[i__] = jpvt[mvidx];
		    jpvt[mvidx] = itemp;
...
/*           Process rejected columns. */
	    ...
	    i__2 = kk - 1;
	    i__3 = *lwork - (mn << 1);
	    cgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*        *-*-*-*-*-*-*-*-*-*-*-*-* */
	...
	i__1 = mn - accptd;
	i__2 = *lwork - (mn << 1);
	cgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &i__1, &
