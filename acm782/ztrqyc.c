/* ztrqyc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ztrqyc(integer *job, integer *m, integer *n, integer *k,
	...
	info)
{
    ...
    static doublecomplex diag, sine;
    static doublereal temp, rcos, smin, smax;
...
/*        Move the best column of A(1:M,M:N) to position M-th. */
	...
	i__1 = *n - mn + 1;
	jj = mn - 1 + izamax(&i__1, &a[mn + mn * a_dim1], lda);
	if (jj > mn) {
	    zswap(m, &a[mn * a_dim1 + 1], &c__1, &a[jj * a_dim1 + 1], &c__1);
	    itemp = jpvt[mn];
	    jpvt[mn] = jpvt[jj];
	    jpvt[jj] = itemp;
	}
...
/*        singular value, and its corresponding left singular vector. */
	...
	for (j = 2; j <= i__1; ++j) {
	    i__2 = j - 1;
	    zlaic1(&c__1, &i__2, &work[1], &smax, &a[j * a_dim1 + 1], &a[j +
	    ...
	    i__2 = j - 1;
	    zlaic1(&c__2, &i__2, &work[mn + 1], &smin, &a[j * a_dim1 + 1], &
...
/*           triangular block A(1:RANK,1:RANK). */
	    ...
	    ztrsv("Upper", "No transpose", "No unit", rank, &a[a_offset],
		    lda, &work[mn + 1], &c__1);

/*           Find the index with largest absolute value in vector */
/*           WORK( MN+1:2*MN ). */

	    jj = izamax(rank, &work[mn + 1], &c__1);

/*           Permut if necessary. */

	    if (jj < *rank && z_abs(&work[mn + jj]) * f > z_abs(&work[mn + *
		    rank])) {
...
/*              RANK->RANK-1. */
		...
		i__1 = *rank;
		for (j = jj + 1; j <= i__1; ++j) {
		    zcopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) * a_dim1
			    + 1], &c__1);
/* L20: */
		}
...
/*        vector RWORK(1:N). */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = min(*m,j);
	    rwork[j] = dznrm2(&i__2, &a[j * a_dim1 + 1], &c__1);
/* L100: */
	}
...
/*        WORK(1:MN) will contain the left singular vector. */
	...
	for (j = 2; j <= i__1; ++j) {
	    i__2 = j - 1;
	    zlaic1(&c__2, &i__2, &work[1], &smin, &a[j * a_dim1 + 1], &a[j +
...
/*           of matrix A were on column RANK+1. */
		...
		zlartg(&a[i__ + ii * a_dim1], &diag, &rcos, &sine, &ctemp);
		diag.r = ctemp.r, diag.i = ctemp.i;
/* L120: */
	    }

	    zlaic1(&c__2, rank, &work[1], &smin, &a[ii * a_dim1 + 1], &diag,
		    &smnrp1, &sine, &cosine);
	    if (smnrp1 >= f * z_abs(&diag)) {
...
/*              ...,II-1->II,II->RANK+1. */
		...
		i__1 = min(mn,ii);
		zcopy(&i__1, &a[ii * a_dim1 + 1], &c__1, &work[mn + 1], &
			c__1);
		i__1 = *rank + 1;
		for (j = ii - 1; j >= i__1; --j) {
/* Computing MIN */
		    i__3 = mn, i__4 = j + 1;
		    i__2 = min(i__3,i__4);
		    zcopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) *
			    a_dim1 + 1], &c__1);
/* L130: */
		}
		i__1 = min(mn,ii);
		zcopy(&i__1, &work[mn + 1], &c__1, &a[(*rank + 1) * a_dim1 +
			1], &c__1);
...
/*              Retriangularize matrix A after permutation. */
		...
		if (*job == 1) {
		    i__1 = min(*m,ii) - *rank;
		    i__2 = *n - *rank;
		    zgret(job, &i__1, &i__2, k, &a[*rank + 1 + (*rank + 1) *
...
/*              Estimate the largest singular value. */
		...
		i__1 = *rank + 1;
		itemp = idamax(&i__1, &rwork[1], &c__1);
		i__1 = *rank + 1;
		smxrp1 = dlasmx(&i__1) * rwork[itemp];
...
/*                 No problem to access WORK(MN+RANK+1) since RANK<MN. */
		    ...
		    zcopy(rank, &work[1], &c__1, &work[mn + 1], &c__1);
...
/*                 Obtain the right singular vector from the left one. */
		    ...
		    i__1 = *rank + 1;
		    ztrsv("Upper", "No transpose", "No unit", &i__1, &a[
			    a_offset], lda, &work[mn + 1], &c__1);

		    i__1 = *rank + 1;
		    jj = izamax(&i__1, &work[mn + 1], &c__1);
...
/*                    JJ+2->JJ+1,...,RANK+1->RANK. */
			...
			i__1 = *rank + 1;
			zcopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[mn +
				1], &c__1);
			i__1 = *rank + 1;
			for (j = jj + 1; j <= i__1; ++j) {
			    zcopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) *
				     a_dim1 + 1], &c__1);
/* L160: */
			}
			i__1 = *rank + 1;
			zcopy(&i__1, &work[mn + 1], &c__1, &a[(*rank + 1) *
				a_dim1 + 1], &c__1);
...
/*                    Retriangularize matrix A after the permutation. */
			...
			if (*job == 1) {
			    i__1 = *rank - jj + 2;
			    i__2 = *n - jj + 1;
			    zhess(job, &i__1, &i__2, k, &a[jj + jj * a_dim1],
...
/*                    vector. */
			...
			for (j = 2; j <= i__1; ++j) {
			    i__2 = j - 1;
			    zlaic1(&c__2, &i__2, &work[1], &smin, &a[j *
...
/*        position RANK+1. */
	...
	i__1 = *n - *rank;
	jj = *rank + idamax(&i__1, &rwork[*rank + 1], &c__1);
	if (jj > *rank + 1 && f * (d__1 = rwork[jj], abs(d__1)) > (d__2 =
		rwork[*rank + 1], abs(d__2))) {

/*           Exchange column JJ to position RANK+1. */

	    i__1 = min(mn,jj);
	    zcopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[mn + 1], &c__1);
	    i__1 = *rank + 1;
	    for (j = jj - 1; j >= i__1; --j) {
/* Computing MIN */
		i__3 = mn, i__4 = j + 1;
		i__2 = min(i__3,i__4);
		zcopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) * a_dim1
			+ 1], &c__1);
/* L200: */
	    }
	    i__1 = min(mn,jj);
	    zcopy(&i__1, &work[mn + 1], &c__1, &a[(*rank + 1) * a_dim1 + 1],
		    &c__1);
...
/*        Computation of the largest singular value of A(1:RANK,1:RANK). */
	...
	for (j = 2; j <= i__1; ++j) {
	    i__2 = j - 1;
	    zlaic1(&c__1, &i__2, &work[mn + 1], &smax, &a[j * a_dim1 + 1], &
...
/*        singular value of A(1:RANK+1,1:RANK+1). */
	...
	zlaic1(&c__1, rank, &work[mn + 1], &smax, &a[(*rank + 1) * a_dim1 +
		1], &a[*rank + 1 + (*rank + 1) * a_dim1], &smxrp1, &sine, &
		cosine);
	zlaic1(&c__2, rank, &work[1], &smin, &a[(*rank + 1) * a_dim1 + 1], &
		a[*rank + 1 + (*rank + 1) * a_dim1], &sminpr, &sine, &cosine);
	zscal(rank, &sine, &work[1], &c__1);
	i__1 = *rank + 1;
	work[i__1].r = cosine.r, work[i__1].i = cosine.i;
	smin = sminpr;
	svlues[3] = smin;
