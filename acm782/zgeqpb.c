/* zgeqpb.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int zgeqpb(integer *job, integer *m, integer *n, integer *k,
	 doublecomplex *a, integer *lda, doublecomplex *c__, integer *ldc,
	integer *jpvt, doublereal *ircond, doublereal *orcond, integer *rank,
	doublereal *svlues, doublecomplex *work, integer *lwork, doublereal *
	rwork, integer *info)
{
    ...
    static integer itemp, wkmin, mvidx, strej, wsize;
    static integer accptd, lacptd;
...
/*     ==================== */
	...
	*info = -15;
    ...
    if ((*info == 0 || *info == -15) && *lwork >= 1) {

/*        Compute the optimal complex workspace. */

	if (*job == 1) {
	    nb = ilaenv(&c__1, "ZGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)6,
		    (ftnlen)1);
/* Computing MAX */
	    i__1 = *n * 3, i__2 = *n * nb;
...
/* Computing MAX */
	    i__1 = ilaenv(&c__1, "ZGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)
		    6, (ftnlen)1), i__2 = ilaenv(&c__1, "ZUNMQR", " ", m, n,
		    &c__0, &c__0, (ftnlen)6, (ftnlen)1);
	    nb = max(i__1,i__2);
/* Computing MAX */
	    i__1 = (mn << 1) + max(*n,*k), i__2 = (mn << 1) + nb * nb + nb *
		    max(*n,*k);
	    wsize = max(i__1,i__2);
	}
...
/* Computing 2nd power */
    ...
    block = nb > 1 && nb >= ilaenv(&c__2, "ZGEQRF", " ", m, n, &c__0, &c__0,
	    (ftnlen)6, (ftnlen)1) && mn >= ilaenv(&c__3, "ZGEQRF", " ", m, n,
	     &c__0, &c__0, (ftnlen)6, (ftnlen)1);
...
/*     *************************************************** */
    ...
    i__1 = *lwork - (mn << 1);
    zgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &c__1, &
	    c__0, &rcond, &lacptd, &jpvt[1], &work[1], &work[mn + 1], &svlues[
	    1], &mxnm, &work[(mn << 1) + 1], &i__1, &rwork[1]);
    if (lacptd == 1) {
	if (lacptd == mn) {
	    *rank = 1;
...
/* Computing MIN */
	...
	i__1 = kk - 1;
	zgeqpw(m, &lwsize, &kb, &i__1, &lacptd, &a[a_offset], lda, &jpvt[1],
...
/*              Accumulate Householder vectors in a block reflector. */
	    ...
	    i__1 = *m - kk + 1;
	    zlarft("Forward", "Columnwise", &i__1, &lacptd, &a[kk + kk *
...
/*              Apply block reflector to A(KK:M,KK+LWSIZE:N). */
		...
		zlarfb("Left", "Conjugate Transpose", "Forward", "Columnwise"
			, &i__1, &i__2, &lacptd, &a[kk + kk * a_dim1], lda, &
			work[(mn << 1) + 1], &lacptd, &a[kk + (kk + lwsize) *
			a_dim1], lda, &work[(mn << 1) + lacptd * lacptd + 1],
			&i__3);
	    }
...
/*                 Apply it to matrix C(KK:M,1:K) from the left. */
		...
		i__1 = *m - kk + 1;
		zlarfb("Left", "Conjugate Transpose", "Forward", "Columnwise"
			, &i__1, k, &lacptd, &a[kk + kk * a_dim1], lda, &work[
			(mn << 1) + 1], &lacptd, &c__[kk + c_dim1], ldc, &
			work[(mn << 1) + lacptd * lacptd + 1], k);
	    } else if (*job == 3 && *k > 0) {
...
/*                 from the right. */
		...
		i__1 = *m - kk + 1;
		zlarfb("Right", "No Transpose", "Forward", "Columnwise", k, &
...
/* Computing MIN */
		...
		for (i__ = kk + lacptd; i__ <= i__1; ++i__) {
		    --mvidx;
		    zswap(m, &a[i__ * a_dim1 + 1], &c__1, &a[mvidx * a_dim1
			    + 1], &c__1);
		    itemp = jpvt[i__];
		    jpvt[i__] = jpvt[mvidx];
		    jpvt[mvidx] = itemp;
...
/*           Process rejected columns. */
	    ...
	    i__2 = kk - 1;
	    i__3 = *lwork - (mn << 1);
	    zgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*        *-*-*-*-*-*-*-*-*-*-*-*-* */
	...
	i__1 = mn - accptd;
	i__2 = *lwork - (mn << 1);
	zgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &i__1, &
		accptd, &rcond, &lacptd, &jpvt[1], &work[1], &work[mn + 1], &
		svlues[1], &mxnm, &work[(mn << 1) + 1], &i__2, &rwork[1]);
	*rank = accptd + lacptd;
