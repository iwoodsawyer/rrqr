/* cmylap.f -- translated by f2c (version 20041007).
...
/* ******************************************************************** */
/* Subroutine */ int cgeqpf(integer *m, integer *n, complex *a, integer *lda,
...
/*     Move initial columns up front */
	...
	if (jpvt[i__] != 0) {
	    if (i__ != itemp) {
		cswap(m, &a[i__ * a_dim1 + 1], &c__1, &a[itemp * a_dim1 + 1],
			 &c__1);
		jpvt[i__] = jpvt[itemp];
		jpvt[itemp] = i__;
	    } else {
...
/*     Compute the QR factorization and update remaining columns */
    ...
    if (itemp > 0) {
	ma = min(itemp,*m);
	cgeqr2(m, &ma, &a[a_offset], lda, &tau[1], &work[1], info);
	if (ma < *n) {
	    i__1 = *n - ma;
	    cunm2r("Left", "Conjugate transpose", m, &i__1, &ma, &a[a_offset]
...
/*        work store the exact column norms. */
	...
	for (i__ = itemp + 1; i__ <= i__1; ++i__) {
	    i__2 = *m - itemp;
	    rwork[i__] = scnrm2(&i__2, &a[itemp + 1 + i__ * a_dim1], &c__1);
...
/*           Determine ith pivot column and swap if necessary */
	    ...
	    i__2 = *n - i__ + 1;
	    pvt = i__ - 1 + isamax(&i__2, &rwork[i__], &c__1);

	    if (pvt != i__) {
		cswap(m, &a[pvt * a_dim1 + 1], &c__1, &a[i__ * a_dim1 + 1], &
			c__1);
		itemp = jpvt[pvt];
		jpvt[pvt] = jpvt[i__];
		jpvt[i__] = itemp;
...
/* Computing MIN */
	    i__3 = i__ + 1;
	    clarfg(&i__2, &aii, &a[min(i__3,*m) + i__ * a_dim1], &c__1, &tau[
		    i__]);
	    i__2 = i__ + i__ * a_dim1;
...
/*              Apply H(i) to A(i:m,i+1:n) from the left */
		...
		i__3 = *n - i__;
		r_cnjg(&q__1, &tau[i__]);
		clarf("Left", &i__2, &i__3, &a[i__ + i__ * a_dim1], &c__1, &
...
/* Computing 2nd power */
			...
			if (*m - i__ > 0) {
			    i__3 = *m - i__;
			    rwork[j] = scnrm2(&i__3, &a[i__ + 1 + j * a_dim1]
				    , &c__1);
			    rwork[*n + j] = rwork[j];
			} else {
			    rwork[j] = 0.f;
...
/* ******************************************************************** */
/* Subroutine */ int cgeqrf(integer *m, integer *n, complex *a, integer *lda,
...
/*     Determine the block size. */
    ...
    nb = ilaenv(&c__1, "CGEQRF", " ", m, n, &c_n1, &c_n1, (ftnlen)6, (ftnlen)
	    1);
    nbmin = 2;
    nx = 0;
    iws = *n;
...
/* Computing MAX */
	i__1 = 0, i__2 = ilaenv(&c__3, "CGEQRF", " ", m, n, &c_n1, &c_n1, (
		ftnlen)6, (ftnlen)1);
	nx = max(i__1,i__2);
	if (nx < k) {
...
/*              determine the minimum value of NB. */
		...
		nb = *lwork / ldwork;
/* Computing MAX */
		i__1 = 2, i__2 = ilaenv(&c__2, "CGEQRF", " ", m, n, &c_n1, &
...
/*           A(i:m,i:i+ib-1) */
	    ...
	    i__3 = *m - i__ + 1;
	    cgeqr2(&i__3, &ib, &a[i__ + i__ * a_dim1], lda, &tau[i__], &work[
		    1], &iinfo);
	    if (i__ + ib <= *n) {
...
/*              H = H(i) H(i+1) . . . H(i+ib-1) */
		...
		i__3 = *m - i__ + 1;
		clarft("Forward", "Columnwise", &i__3, &ib, &a[i__ + i__ *
...
/*     Use unblocked code to factor the last or only block. */
	...
	i__2 = *m - i__ + 1;
	i__1 = *n - i__ + 1;
	cgeqr2(&i__2, &i__1, &a[i__ + i__ * a_dim1], lda, &tau[i__], &work[1]
		, &iinfo);
    }
...
/* ******************************************************************** */
/* Subroutine */ int cgeqr2(integer *m, integer *n, complex *a, integer *lda,
...
/* Computing MIN */
	i__3 = i__ + 1;
	clarfg(&i__2, &a[i__ + i__ * a_dim1], &a[min(i__3,*m) + i__ * a_dim1]
		, &c__1, &tau[i__]);
	if (i__ < *n) {
...
/*           Apply H(i)' to A(i:m,i+1:n) from the left */
	    ...
	    i__3 = *n - i__;
	    r_cnjg(&q__1, &tau[i__]);
	    clarf("Left", &i__2, &i__3, &a[i__ + i__ * a_dim1], &c__1, &q__1,
