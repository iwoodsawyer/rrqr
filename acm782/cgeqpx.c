/* cgeqpx.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int cgeqpx(integer *job, integer *m, integer *n, integer *k,
...
/*     ============= */
    ...
    cgeqpb(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1],
...
/*     ============== */
    ...
    if (*rank > 0) {
	ctrqpx(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
		, ircond, orcond, rank, &svlues[1], &work[1], &rwork[1], info)
		;
    }

    work[1].r = wsize, work[1].i = 0.f;
    return 0;
