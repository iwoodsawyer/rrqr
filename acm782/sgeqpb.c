/* sgeqpb.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sgeqpb(integer *job, integer *m, integer *n, integer *k,
	...
	integer *lwork, integer *info)
{
    ...
    static real rcond;
    static integer itemp, wkmin, mvidx, strej;
    static integer wsize, accptd, lacptd;
...
/*        Compute the optimal workspace size. */
	...
	if (*job == 1) {
	    nb = ilaenv(&c__1, "SGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)6,
		    (ftnlen)1);
/* Computing MAX */
	    i__1 = *n * 3, i__2 = *n * nb;
...
/* Computing MAX */
	    i__1 = ilaenv(&c__1, "SGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)
		    6, (ftnlen)1), i__2 = ilaenv(&c__1, "SORMQR", " ", m, n,
...
/* Computing 2nd power */
    ...
    block = nb > 1 && nb >= ilaenv(&c__2, "SGEQRF", " ", m, n, &c__0, &c__0,
	    (ftnlen)6, (ftnlen)1) && mn >= ilaenv(&c__3, "SGEQRF", " ", m, n,
	     &c__0, &c__0, (ftnlen)6, (ftnlen)1);
...
/*     *************************************************** */
    ...
    i__1 = *lwork - (mn << 1);
    sgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &c__1, &
...
/* Computing MIN */
	...
	i__1 = kk - 1;
	sgeqpw(m, &lwsize, &kb, &i__1, &lacptd, &a[a_offset], lda, &jpvt[1],
...
/*              Accumulate Householder vectors in a block reflector. */
	    ...
	    i__1 = *m - kk + 1;
	    slarft("Forward", "Columnwise", &i__1, &lacptd, &a[kk + kk *
...
/*              Apply block reflector to A(KK:M,KK+LWSIZE:N). */
		...
		i__2 = *n - kk - lwsize + 1;
		i__3 = *n - kk - lwsize + 1;
		slarfb("Left", "Transpose", "Forward", "Columnwise", &i__1, &
...
/*                 from the right. */
		...
		i__1 = *m - kk + 1;
		slarfb("Right", "No Transpose", "Forward", "Columnwise", k, &
...
/* Computing MIN */
		...
		for (i__ = kk + lacptd; i__ <= i__1; ++i__) {
		    --mvidx;
		    sswap(m, &a[i__ * a_dim1 + 1], &c__1, &a[mvidx * a_dim1
			    + 1], &c__1);
		    itemp = jpvt[i__];
		    jpvt[i__] = jpvt[mvidx];
		    jpvt[mvidx] = itemp;
...
/*           Process rejected columns. */
	    ...
	    i__2 = kk - 1;
	    i__3 = *lwork - (mn << 1);
	    sgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*        *-*-*-*-*-*-*-*-*-*-*-*-* */
	...
	i__1 = mn - accptd;
	i__2 = *lwork - (mn << 1);
	sgeqpc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &i__1, &
