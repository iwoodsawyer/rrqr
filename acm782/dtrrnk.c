/* dtrrnk.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int dtrrnk(integer *n, doublereal *r__, integer *ldr,
	doublereal *rcond, integer *rank, doublereal *work, integer *info)
{
    ...
    /* Local variables */
    static integer i__;
    static doublereal c1, c2, s1, s2, smin, smax;
...
/*     Determine RANK using incremental condition estimation. */
    ...
    work[1] = 1.;
    work[*n + 1] = 1.;
    smax = (d__1 = r__[r_dim1 + 1], abs(d__1));
    smin = smax;
    if ((d__1 = r__[r_dim1 + 1], abs(d__1)) == 0.) {
	*rank = 0;
	goto L30;
    } else {
	*rank = 1;
...
L10:
    if (*rank < *n) {
	i__ = *rank + 1;
	dlaic1(&c__2, rank, &work[1], &smin, &r__[i__ * r_dim1 + 1], &r__[
		i__ + i__ * r_dim1], &sminpr, &s1, &c1);
	dlaic1(&c__1, rank, &work[*n + 1], &smax, &r__[i__ * r_dim1 + 1], &
