/* dlauc1.f -- translated by f2c (version 20041007).
...
logical dlauc1(integer *k, doublereal *x, doublereal *smin, doublereal *w,
	doublereal *gamma, doublereal *thresh)
{
    ...
    static doublereal cosine, sminpr;
...
/*     for A(K,K). */
    ...
    i__1 = *k - 1;
    dlaic1(&c__2, &i__1, &x[1], smin, &w[1], gamma, &sminpr, &sine, &cosine);
    if (*thresh > sminpr) {
	ret_val = FALSE_;
    } else {
	i__1 = *k - 1;
