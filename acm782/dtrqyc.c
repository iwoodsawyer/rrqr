/* dtrqyc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int dtrqyc(integer *job, integer *m, integer *n, integer *k,
	...
	*rcnrp1, doublereal *work, integer *info)
{
    ...
    static doublereal diag, sine, temp, smin, smax;
    static integer nctba;
    static integer itemp;
    static doublereal smnrp1;
    static doublereal smxrp1;
    static doublereal cosine;
    static doublereal sminpr, smaxpr, rdummy[1]	/* was [1][1] */;
...
/*        Move the best column of A(1:M,M:N) to position M-th. */
	...
	i__1 = *n - mn + 1;
	jj = mn - 1 + idamax(&i__1, &a[mn + mn * a_dim1], lda);
	if (jj > mn) {
	    dswap(m, &a[mn * a_dim1 + 1], &c__1, &a[jj * a_dim1 + 1], &c__1);
	    itemp = jpvt[mn];
	    jpvt[mn] = jpvt[jj];
	    jpvt[jj] = itemp;
	}
...
/*        value, and its corresponding left singular vector. */
	...
	for (j = 2; j <= i__1; ++j) {
	    i__2 = j - 1;
	    dlaic1(&c__1, &i__2, &work[1], &smax, &a[j * a_dim1 + 1], &a[j +
	    ...
	    i__2 = j - 1;
	    dlaic1(&c__2, &i__2, &work[mn + 1], &smin, &a[j * a_dim1 + 1], &
...
/*           triangular block A(1:RANK,1:RANK). */
	    ...
	    dtrsv("Upper", "No transpose", "No unit", rank, &a[a_offset],
		    lda, &work[mn + 1], &c__1);

/*           Find the index with largest absolute value in vector */
/*           WORK( MN+1:2*MN ). */

	    jj = idamax(rank, &work[mn + 1], &c__1);

/*           Permut if necessary. */

	    if (jj < *rank && (d__1 = work[mn + jj], abs(d__1)) * f > (d__2 =
		    work[mn + *rank], abs(d__2))) {
...
/*              RANK->RANK-1. */
		...
		i__1 = *rank;
		for (j = jj + 1; j <= i__1; ++j) {
		    dcopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) * a_dim1
			    + 1], &c__1);
/* L20: */
		}
...
/*        vector WORK(1:N). */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = min(*m,j);
	    work[j] = dnrm2(&i__2, &a[j * a_dim1 + 1], &c__1);
/* L100: */
	}
...
/*        WORK(N+1:N+MN) will contain the left singular vector. */
	...
	for (j = 2; j <= i__1; ++j) {
	    i__2 = j - 1;
	    dlaic1(&c__2, &i__2, &work[*n + 1], &smin, &a[j * a_dim1 + 1], &
		    a[j + j * a_dim1], &sminpr, &sine, &cosine);
	    i__2 = j - 1;
	    dscal(&i__2, &sine, &work[*n + 1], &c__1);
	    work[*n + j] = cosine;
	    smin = sminpr;
/* L110: */
	}
...
/*           of matrix A were on column RANK+1. */
		...
		dlartg(&a[i__ + ii * a_dim1], &diag, &cosine, &sine, &temp);
		diag = temp;
/* L120: */
	    }

	    dlaic1(&c__2, rank, &work[*n + 1], &smin, &a[ii * a_dim1 + 1], &
		    diag, &smnrp1, &sine, &cosine);
	    if (smnrp1 >= f * abs(diag)) {
...
/*              ...,II-1->II,II->RANK+1. */
		...
		i__1 = min(mn,ii);
		dcopy(&i__1, &a[ii * a_dim1 + 1], &c__1, &work[*n + mn + 1],
			&c__1);
		i__1 = *rank + 1;
		for (j = ii - 1; j >= i__1; --j) {
/* Computing MIN */
		    i__3 = mn, i__4 = j + 1;
		    i__2 = min(i__3,i__4);
		    dcopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) *
			    a_dim1 + 1], &c__1);
/* L130: */
		}
		i__1 = min(mn,ii);
		dcopy(&i__1, &work[*n + mn + 1], &c__1, &a[(*rank + 1) *
			a_dim1 + 1], &c__1);
...
/*              Retriangularize matrix A after permutation. */
		...
		if (*job == 1) {
		    i__1 = min(*m,ii) - *rank;
		    i__2 = *n - *rank;
		    dgret(job, &i__1, &i__2, k, &a[*rank + 1 + (*rank + 1) *
...
/*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* */
		...
		i__1 = *rank + 1;
		itemp = idamax(&i__1, &work[1], &c__1);
		i__1 = *rank + 1;
		smxrp1 = dlasmx(&i__1) * work[itemp];

/*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* */
/*              * Estimate the right singular vector  * */
/*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* */

		if (smnrp1 > smxrp1 * 100. * dlamch("Safe minimum")) {

/*                 Matrix is not singular or not nearly singular. */

/*                 First, end the estimation of the left singular vector. */

		    dcopy(rank, &work[*n + 1], &c__1, &work[*n + mn + 1], &
			    c__1);
		    dscal(rank, &sine, &work[*n + mn + 1], &c__1);
		    work[*n + mn + *rank + 1] = cosine;

/*                 Obtain the right singular vector from the left one. */

		    i__1 = *rank + 1;
		    dtrsv("Upper", "No transpose", "No unit", &i__1, &a[
			    a_offset], lda, &work[*n + mn + 1], &c__1);

		    i__1 = *rank + 1;
		    jj = idamax(&i__1, &work[*n + mn + 1], &c__1);
...
/*                    JJ+2->JJ+1,...,RANK+1->RANK. */
			...
			i__1 = *rank + 1;
			dcopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[*n +
				mn + 1], &c__1);
			i__1 = *rank + 1;
			for (j = jj + 1; j <= i__1; ++j) {
			    dcopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) *
				     a_dim1 + 1], &c__1);
/* L160: */
			}
			i__1 = *rank + 1;
			dcopy(&i__1, &work[*n + mn + 1], &c__1, &a[(*rank +
				1) * a_dim1 + 1], &c__1);
...
/*                    Retriangularize matrix A after the permutation. */
			...
			if (*job == 1) {
			    i__1 = *rank - jj + 2;
			    i__2 = *n - jj + 1;
			    dhess(job, &i__1, &i__2, k, &a[jj + jj * a_dim1],
...
/*                    vector. */
			...
			for (j = 2; j <= i__1; ++j) {
			    i__2 = j - 1;
			    dlaic1(&c__2, &i__2, &work[*n + 1], &smin, &a[j *
...
/*        position RANK+1. */
	...
	i__1 = *n - *rank;
	jj = *rank + idamax(&i__1, &work[*rank + 1], &c__1);
	if (jj > *rank + 1 && f * (d__1 = work[jj], abs(d__1)) > (d__2 = work[
		*rank + 1], abs(d__2))) {

/*           Exchange column JJ to position RANK+1. */

	    i__1 = min(mn,jj);
	    dcopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[*n + mn + 1], &
		    c__1);
	    i__1 = *rank + 1;
	    for (j = jj - 1; j >= i__1; --j) {
/* Computing MIN */
		i__3 = mn, i__4 = j + 1;
		i__2 = min(i__3,i__4);
		dcopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) * a_dim1
			+ 1], &c__1);
/* L200: */
	    }
	    i__1 = min(mn,jj);
	    dcopy(&i__1, &work[*n + mn + 1], &c__1, &a[(*rank + 1) * a_dim1
		    + 1], &c__1);
...
/*        singular value of A(1:RANK+1,1:RANK+1). */
	...
	dlaic1(&c__2, rank, &work[*n + 1], &smin, &a[(*rank + 1) * a_dim1 +
		1], &a[*rank + 1 + (*rank + 1) * a_dim1], &sminpr, &sine, &
		cosine);
	dscal(rank, &sine, &work[*n + 1], &c__1);
