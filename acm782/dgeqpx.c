/* dgeqpx.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int dgeqpx(integer *job, integer *m, integer *n, integer *k,
...
/*     ============== */
    ...
    if (*rank > 0) {
	dtrqpx(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
		, ircond, orcond, rank, &svlues[1], &work[1], lwork, info);
    }
