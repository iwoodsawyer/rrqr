/* ctrqxc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ctrqxc(integer *job, integer *m, integer *n, integer *k,
...
/*     the largest singular value in order to pass it to Chan-II. */
    ...
    i__1 = *rank;
    for (j = 1; j <= i__1; ++j) {
	rwork[j] = scnrm2(&j, &a[j * a_dim1 + 1], &c__1);
/* L10: */
    }
...
/*        Apply Golub-I for the stage RANK. */
	...
	cglbif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
		, &c_b6, rank, &permut, &work[1], &rwork[mn + 1], info);

/*        If necessary, update the contents of WORK(RANK). */

	if (permut) {
	    rwork[*rank] = scnrm2(rank, &a[*rank * a_dim1 + 1], &c__1);
	}
...
/*           Apply Golub-I for the stage RANK+1. */
	    ...
	    i__1 = *rank + 1;
	    cglbif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*           easier. */
	    ...
	    i__1 = *rank + 1;
	    rwork[*rank + 1] = scnrm2(&i__1, &a[(*rank + 1) * a_dim1 + 1], &
		    c__1);
...
/*           on block A(1:RANK+1,1:RANK+1). */
	    ...
	    i__1 = *rank + 1;
	    ccniif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*        Apply Chan-II for the stage RANK on block A(1:RANK,1:RANK). */
	...
	ccniif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
...
/*     Computation of the largest singular value of A(1:RANK,1:RANK). */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	claic1(&c__1, &i__2, &work[1], &smax, &a[j * a_dim1 + 1], &a[j + j *
	...
	smax = smaxpr;
	i__2 = j - 1;
	claic1(&c__2, &i__2, &work[mn + 1], &smin, &a[j * a_dim1 + 1], &a[j
...
/*     singular value of A(1:RANK+1,1:RANK+1). */
    ...
    if (*rank < mn) {
	claic1(&c__1, rank, &work[1], &smax, &a[(*rank + 1) * a_dim1 + 1], &
		a[*rank + 1 + (*rank + 1) * a_dim1], &smaxpr, &sine, &cosine);
	smax = smaxpr;
	claic1(&c__2, rank, &work[mn + 1], &smin, &a[(*rank + 1) * a_dim1 +
...
/* Subroutine */ int cglbif(integer *job, integer *m, integer *n, integer *k,
	 complex *a, integer *lda, complex *c__, integer *ldc, integer *jpvt,
	real *f, integer *rank, logical *permut, complex *work, real *rwork,
	integer *info)
{
    /* System generated locals */
...
/*     and store them in vector RWORK(RANK:N). */
    ...
    for (j = *rank; j <= i__1; ++j) {
	i__2 = min(*m,j) - *rank + 1;
	rwork[j] = scnrm2(&i__2, &a[*rank + j * a_dim1], &c__1);
/* L10: */
    }
...
/*     A(RANK:M,RANK:N). Use the data stored in vector RWORK(RANK:N). */
    ...
    i__1 = *n - *rank + 1;
    jj = *rank - 1 + isamax(&i__1, &rwork[*rank], &c__1);

/*     Determine if a permutation must occur. */

    *permut = jj > *rank && (r__1 = rwork[jj], dabs(r__1)) * *f > (r__2 =
	    rwork[*rank], dabs(r__2));
...
/*        to store temporal data. */
	...
	i__1 = min(mn,jj);
	ccopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[1], &c__1);
	i__1 = *rank;
	for (j = jj - 1; j >= i__1; --j) {
/* Computing MIN */
	    i__3 = mn, i__4 = j + 1;
	    i__2 = min(i__3,i__4);
	    ccopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) * a_dim1 + 1],
		     &c__1);
/* L20: */
	}
	i__1 = min(mn,jj);
	ccopy(&i__1, &work[1], &c__1, &a[*rank * a_dim1 + 1], &c__1);
...
/*        Retriangularize matrix A after the permutation. */
	...
	if (*job == 1) {
	    ...
	    i__2 = *n - *rank + 1;
	    cgret(job, &i__1, &i__2, k, &a[*rank + *rank * a_dim1], lda,
...
/* Subroutine */ int ccniif(integer *job, integer *m, integer *n, integer *k,
	 complex *a, integer *lda, complex *c__, integer *ldc, integer *jpvt,
	real *vnorm, real *f, integer *rank, logical *permut, complex *work,
	real *rwork, integer *info)
{
    /* System generated locals */
    integer a_dim1, a_offset, c_dim1, c_offset, i__1, i__2;
...
/*     Save left singular vector in vector WORK(1:RANK). */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	claic1(&c__2, &i__2, &work[1], &smin, &a[j * a_dim1 + 1], &a[j + j *
...
/*        WORK(1:RANK). */
	...
	ctrsv("Upper", "No transpose", "No unit", rank, &a[a_offset], lda, &
		work[1], &c__1);

/*        Find the index with largest absolute value in vector */
/*        WORK(1:RANK). */

	jj = icamax(rank, &work[1], &c__1);

/*        Determine if a permutation must occur. */

	*permut = jj < *rank && c_abs(&work[jj]) * *f > c_abs(&work[*rank]);
...
/*           RANK->RANK-1. Use vector WORK to store temporal data. */
	    ...
	    i__1 = *rank;
	    for (j = jj + 1; j <= i__1; ++j) {
		ccopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) * a_dim1 + 1]
			, &c__1);
/* L20: */
	    }
...
/*           Retriangularize matrix A after the permutation. */
		...
		i__1 = *rank - jj + 1;
		i__2 = *n - jj + 1;
		chess(job, &i__1, &i__2, k, &a[jj + jj * a_dim1], lda, &c__[
...
/* Subroutine */ int cgret(integer *job, integer *m, integer *n, integer *k,
	complex *a, integer *lda, complex *c__, integer *ldc, complex *work,
	real *rwork, integer *info)
{
    /* System generated locals */
...
/*        Givens Rotations in vectors WORK and RWORK. */
	...
	for (i__ = *m; i__ >= 2; --i__) {
	    clartg(&a[i__ - 1 + a_dim1], &a[i__ + a_dim1], &rwork[i__], &
		    work[i__], &r__);
	    i__1 = i__ - 1 + a_dim1;
...
/* Computing MIN */
		...
		i__3 = j + jb - i__;
		crot(&i__3, &a[i__ - 1 + i__ * a_dim1], lda, &a[i__ + i__ *
...
/* Computing MIN */
		...
		crot(&jb, &a[i__ - 1 + j * a_dim1], lda, &a[i__ + j * a_dim1]
...
/* Computing MIN */
		...
		jb = min(i__5,i__3);
		for (i__ = *m; i__ >= 2; --i__) {
		    crot(&jb, &c__[i__ - 1 + j * c_dim1], ldc, &c__[i__ + j *
...
/*           from the right. */
		...
		crot(k, &c__[(i__ - 1) * c_dim1 + 1], &c__1, &c__[i__ *
			c_dim1 + 1], &c__1, &rwork[i__], &q__1);
/* L70: */
	    }
	}
    } else {
...
/*           Compute the rotation parameters and update column 1 of A. */
	    ...
	    clartg(&a[itemp + a_dim1], &a[i__ + a_dim1], &cosine, &sine, &
		    r__);
	    i__1 = itemp + a_dim1;
	    a[i__1].r = r__.r, a[i__1].i = r__.i;
	    i__1 = i__ + a_dim1;
...
/*           Update columns I:N of matrix A. */
	    ...
	    i__1 = *n - i__ + 1;
	    crot(&i__1, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ * a_dim1]
		    , lda, &cosine, &sine);
...
/*              Apply the previously computed rotations from the left. */
		...
		crot(k, &c__[itemp + c_dim1], ldc, &c__[i__ + c_dim1], ldc, &
			cosine, &sine);
	    } else if (*job == 3 && *k > 0) {
...
/*              from the right. */
		...
		r_cnjg(&q__1, &sine);
		crot(k, &c__[itemp * c_dim1 + 1], &c__1, &c__[i__ * c_dim1 +
			1], &c__1, &cosine, &q__1);
	    }
/* L90: */
	}
...
/* Subroutine */ int chess(integer *job, integer *m, integer *n, integer *k,
	complex *a, integer *lda, complex *c__, integer *ldc, complex *work,
	real *rwork, integer *info)
{
    /* System generated locals */
...
/* Computing MIN */
		...
		itemp = i__ - 1;
		clartg(&a[itemp + itemp * a_dim1], &a[i__ + itemp * a_dim1],
		...
		i__4 = j + jb - i__;
		crot(&i__4, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ *
			a_dim1], lda, &rwork[i__], &work[i__]);
/* L30: */
	    }
/* L10: */
...
/*           Update columns I-1:N of matrix A. */
	    ...
	    i__2 = *n - i__ + 1;
	    crot(&i__2, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ * a_dim1]
		    , lda, &cosine, &sine);
