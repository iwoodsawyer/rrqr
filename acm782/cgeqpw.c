/* cgeqpw.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int cgeqpw(integer *m, integer *lwsize, integer *nb,
	integer *offset, integer *lacptd, complex *a, integer *lda, integer *
	jpvt, real *ircond, complex *x, real *smin, real *mxnm, complex *tau,
...
/*     LWSIZE entries of WORK) for the first batch of columns. */
	...
	i__2 = *m - *offset;
	rwork[i__] = scnrm2(&i__2, &a[*offset + 1 + (*offset + i__) * a_dim1]
		, &c__1);
...
/*        ========================= */
    i__1 = *lwsize - *lacptd;
    pvtidx = *offset + *lacptd + isamax(&i__1, &rwork[*lacptd + 1], &c__1);
    k = *offset + *lacptd + 1;

/*        Exchange current column and pivot column. */

    if (pvtidx != k) {
	cswap(m, &a[pvtidx * a_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
	i1 = jpvt[pvtidx];
	jpvt[pvtidx] = jpvt[k];
	jpvt[k] = i1;
...
/*        GAMMA of matrix A should elementary reflector be applied. */
    ...
    if (temp == 0.f) {
	r__1 = -rwork[k - *offset];
	gamma.r = r__1, gamma.i = 0.f;
    } else {
	r__1 = -r_sign(&rwork[k - *offset], &temp);
	gamma.r = r__1, gamma.i = 0.f;
    }
...
/*        ===================================== */
    r__1 = smax * *ircond;
    if (clauc1(&k, &x[1], smin, &a[k * a_dim1 + 1], &gamma, &r__1)) {
...
/*           Generate Householder vector. */
	...
	if (k < *m) {
	    i__1 = *m - k + 1;
	    clarfg(&i__1, &a[k + k * a_dim1], &a[k + 1 + k * a_dim1], &c__1,
		    &tau[k]);
	} else {
	    clarfg(&c__1, &a[*m + k * a_dim1], &a[*m + k * a_dim1], &c__1, &
		    tau[k]);
	}
...
/*           Apply Householder reflection to A(k:m,k+1:lwsize). */
	...
	if (*lacptd < *lwsize) {
	    ...
	    i__2 = *lwsize - *lacptd;
	    r_cnjg(&q__1, &tau[k]);
	    clarf("Left", &i__1, &i__2, &a[k + k * a_dim1], &c__1, &q__1, &a[
...
/*           Update partial column norms. */
	...
	if (k < lastk) {
		...
		if (rwork[i__] != 0.f) {
/* Computing 2nd power */
		    r__1 = c_abs(&a[k + (*offset + i__) * a_dim1]) / rwork[
			    i__];
		    temp = 1.f - r__1 * r__1;
		    temp = dmax(temp,0.f);
/* Computing 2nd power */
		    r__1 = rwork[i__] / rwork[i__ + *lwsize];
		    temp2 = temp * .05f * (r__1 * r__1) + 1.f;
		    if (temp2 == 1.f) {
			i__2 = *m - k;
			rwork[i__] = scnrm2(&i__2, &a[k + 1 + (*offset + i__)
				 * a_dim1], &c__1);
