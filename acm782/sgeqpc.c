/* sgeqpc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sgeqpc(integer *job, integer *m, integer *n, integer *k,
	 real *a, integer *lda, real *c__, integer *ldc, integer *dsrd,
	integer *offset, real *ircond, integer *lacptd, integer *jpvt, real *
	tau, real *x, real *svlues, real *mxnm, real *work, integer *lwork)
{
    ...
    static real sine, temp, smin, smax, temp2;
...
/*     work store the exact column norms. */
	...
	i__2 = *m - *offset;
	work[i__] = snrm2(&i__2, &a[*offset + 1 + i__ * a_dim1], &c__1);
	work[*n + i__] = work[i__];
/* L10: */
    }
...
/*        Generate elementary reflector H(i). */
	...
	if (i__ < *m) {
	    i__2 = *m - i__ + 1;
	    slarfg(&i__2, &a[i__ + i__ * a_dim1], &a[i__ + 1 + i__ * a_dim1],
		     &c__1, &tau[i__]);
	} else {
	    slarfg(&c__1, &a[*m + *m * a_dim1], &a[*m + *m * a_dim1], &c__1,
		    &tau[*m]);
	}
...
/*           Apply H(I) to C(I:M,1:K) from the left. */
	    ...
	    i__2 = *m - i__ + 1;
	    slarf("Left", &i__2, k, &a[i__ + i__ * a_dim1], &c__1, &tau[i__],
...
/*           Apply the transpose of H(I) to C(1:K,I:M) from the right. */
	    ...
	    i__2 = *m - i__ + 1;
	    slarf("Right", k, &i__2, &a[i__ + i__ * a_dim1], &c__1, &tau[i__]
...
/*        Check new column for independence. */
	...
	if (i__ == 1) {
	    *mxnm = (r__1 = a[a_dim1 + 1], dabs(r__1));
	    smin = *mxnm;
	    smax = *mxnm;
	    x[1] = 1.f;
	    if (*mxnm > 0.f) {
		*lacptd = 1;
	    ...
	    r__1 = smaxpr * *ircond;
	    if (slauc1(&i__, &x[1], &smin, &a[i__ * a_dim1 + 1], &a[i__ +
		    i__ * a_dim1], &r__1)) {
...
/*           Factor remaining columns. */
	    ...
	    i__1 = *m - i__;
	    i__2 = *n - i__;
	    sgeqrf(&i__1, &i__2, &a[i__ + 1 + (i__ + 1) * a_dim1], lda, &tau[
...
/*              Apply them to C(I+1:M,1:K) from the left. */
		...
		i__1 = *m - i__;
		i__2 = mn - i__;
		sormqr("Left", "Transpose", &i__1, k, &i__2, &a[i__ + 1 + (
...
/*              right. */
		...
		i__1 = *m - i__;
		i__2 = mn - i__;
		sormqr("Right", "No Transpose", k, &i__1, &i__2, &a[i__ + 1
...
/* Computing MAX */
	...
	for (i__ = max(i__1,i__2); i__ <= i__3; ++i__) {
	    i__1 = i__ - 1;
	    slaic1(&c__2, &i__1, &x[1], &smin, &a[i__ * a_dim1 + 1], &a[i__
