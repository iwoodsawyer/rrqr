/* zlauc1.f -- translated by f2c (version 20041007).
...
logical zlauc1(integer *k, doublecomplex *x, doublereal *smin, doublecomplex
	*w, doublecomplex *gamma, doublereal *thresh)
{
...
/*     for A(K,K). */
    ...
    i__1 = *k - 1;
    zlaic1(&c__2, &i__1, &x[1], smin, &w[1], gamma, &sminpr, &sine, &cosine);
    if (*thresh > sminpr) {
	ret_val = FALSE_;
    } else {
	i__1 = *k - 1;
