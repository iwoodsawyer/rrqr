/* zgeqpy.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int zgeqpy(integer *job, integer *m, integer *n, integer *k,
...
/*     ============= */
    ...
    zgeqpb(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1],
	    ircond, orcond, rank, &svlues[1], &work[1], lwork, &rwork[1],
	    info);
    wsize = work[1].r;
...
/*     ============== */
    ...
    if (*rank > 0) {
	ztrqpy(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
		, ircond, orcond, rank, &svlues[1], &work[1], &rwork[1], info)
		;
    }

    work[1].r = wsize, work[1].i = 0.;
    return 0;
