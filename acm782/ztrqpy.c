/* ztrqpy.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ztrqpy(integer *job, integer *m, integer *n, integer *k,
	 doublecomplex *a, integer *lda, doublecomplex *c__, integer *ldc,
	integer *jpvt, doublereal *ircond, doublereal *orcond, integer *rank,
	doublereal *svlues, doublecomplex *work, doublereal *rwork, integer *
	info)
{
...
/*     .. Executable Statements .. */
    ...
    /* Function Body */
    mn = min(*m,*n);
    bsprqr_1.nb = ilaenv(&c__1, "ZGEQRF", " ", m, n, &c__0, &c__0, (ftnlen)6,
	     (ftnlen)1);
...
/*     Compute the initial estimate for the rank. */
    ...
    ztrrnk(&mn, &a[a_offset], lda, &rcond, rank, &work[1], info);
...
/*     Get tighter bounds for the value RANK. */
    ...
    ztrqyc(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1],
	    rank, &svlues[1], &rcnr, &rcnrp1, &work[1], &rwork[1], info);
    oinfo = 0;
    if (*info != 0) {
	oinfo = *info;
    }
