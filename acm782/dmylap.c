/* dmylap.f -- translated by f2c (version 20041007).
...
/* ******************************************************************** */
/* Subroutine */ int dgeqpf(integer *m, integer *n, doublereal *a, integer *
...
/*     Move initial columns up front */
	...
	if (jpvt[i__] != 0) {
	    if (i__ != itemp) {
		dswap(m, &a[i__ * a_dim1 + 1], &c__1, &a[itemp * a_dim1 + 1],
			 &c__1);
		jpvt[i__] = jpvt[itemp];
		jpvt[itemp] = i__;
	    } else {
...
/*     Compute the QR factorization and update remaining columns */
    ...
    if (itemp > 0) {
	ma = min(itemp,*m);
	dgeqr2(m, &ma, &a[a_offset], lda, &tau[1], &work[1], info);
	if (ma < *n) {
	    i__1 = *n - ma;
	    dorm2r("Left", "Transpose", m, &i__1, &ma, &a[a_offset], lda, &
...
/*        work store the exact column norms. */
	...
	for (i__ = itemp + 1; i__ <= i__1; ++i__) {
	    i__2 = *m - itemp;
	    work[i__] = dnrm2(&i__2, &a[itemp + 1 + i__ * a_dim1], &c__1);
	    work[*n + i__] = work[i__];
/* L20: */
	}
...
/*           Generate elementary reflector H(i) */
	    ...
	    if (i__ < *m) {
		i__2 = *m - i__ + 1;
		dlarfg(&i__2, &a[i__ + i__ * a_dim1], &a[i__ + 1 + i__ *
			a_dim1], &c__1, &tau[i__]);
	    } else {
		dlarfg(&c__1, &a[*m + *m * a_dim1], &a[*m + *m * a_dim1], &
			c__1, &tau[*m]);
	    }
...
/* ******************************************************************** */
/* Subroutine */ int dgeqrf(integer *m, integer *n, doublereal *a, integer *
...
/*     Determine the block size. */
    ...
    nb = ilaenv(&c__1, "DGEQRF", " ", m, n, &c_n1, &c_n1, (ftnlen)6, (ftnlen)
	    1);
    nbmin = 2;
    nx = 0;
    iws = *n;
...
/* Computing MAX */
	i__1 = 0, i__2 = ilaenv(&c__3, "DGEQRF", " ", m, n, &c_n1, &c_n1, (
		ftnlen)6, (ftnlen)1);
	nx = max(i__1,i__2);
	if (nx < k) {
...
/*              determine the minimum value of NB. */
		...
		nb = *lwork / ldwork;
/* Computing MAX */
		i__1 = 2, i__2 = ilaenv(&c__2, "DGEQRF", " ", m, n, &c_n1, &
...
/*              H = H(i) H(i+1) . . . H(i+ib-1) */
		...
		i__3 = *m - i__ + 1;
		dlarft("Forward", "Columnwise", &i__3, &ib, &a[i__ + i__ *
...
/*              Apply H' to A(i:m,i+ib:n) from the left */
		...
		i__3 = *m - i__ + 1;
		i__4 = *n - i__ - ib + 1;
		dlarfb("Left", "Transpose", "Forward", "Columnwise", &i__3, &
...
/* ******************************************************************** */
/* Subroutine */ int dgeqr2(integer *m, integer *n, doublereal *a, integer *
...
/* Computing MIN */
	i__3 = i__ + 1;
	dlarfg(&i__2, &a[i__ + i__ * a_dim1], &a[min(i__3,*m) + i__ * a_dim1]
		, &c__1, &tau[i__]);
	if (i__ < *n) {
