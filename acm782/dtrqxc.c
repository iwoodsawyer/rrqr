/* dtrqxc.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int dtrqxc(integer *job, integer *m, integer *n, integer *k,
	 doublereal *a, integer *lda, doublereal *c__, integer *ldc, integer *
	jpvt, integer *rank, doublereal *svlues, doublereal *rcnr, doublereal
	*rcnrp1, doublereal *work, integer *info)
{
    ...
    static doublereal sine, smin, smax;
    static doublereal smxrp1;
    static integer nacptd;
    static doublereal cosine, sminpr, smaxpr;
...
/*     the largest singular value in order to pass it to Chan-II. */
    ...
    i__1 = *rank;
    for (j = 1; j <= i__1; ++j) {
	work[j] = dnrm2(&j, &a[j * a_dim1 + 1], &c__1);
/* L10: */
    }
...
/*        Apply Golub-I for the stage RANK. */
	...
	dglbif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
		, &c_b5, rank, &permut, &work[mn + 1], info);

/*        If necessary, update the contents of WORK(RANK). */

	if (permut) {
	    work[*rank] = dnrm2(rank, &a[*rank * a_dim1 + 1], &c__1);
	}
...
/*           Apply Golub-I for the stage RANK+1. */
	    ...
	    i__1 = *rank + 1;
	    dglbif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*           easier. */
	    ...
	    i__1 = *rank + 1;
	    work[*rank + 1] = dnrm2(&i__1, &a[(*rank + 1) * a_dim1 + 1], &
		    c__1);
...
/*           on block A(1:RANK+1,1:RANK+1). */
	    ...
	    i__1 = *rank + 1;
	    dcniif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &
...
/*        Apply Chan-II for the stage RANK on block A(1:RANK,1:RANK). */
	...
	dcniif(job, m, n, k, &a[a_offset], lda, &c__[c_offset], ldc, &jpvt[1]
...
/*     singular value of A(1:RANK,1:RANK). */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	dlaic1(&c__1, &i__2, &work[1], &smax, &a[j * a_dim1 + 1], &a[j + j *
		a_dim1], &smaxpr, &sine, &cosine);
	i__2 = j - 1;
	dscal(&i__2, &sine, &work[1], &c__1);
	work[j] = cosine;
	smax = smaxpr;
	i__2 = j - 1;
	dlaic1(&c__2, &i__2, &work[mn + 1], &smin, &a[j * a_dim1 + 1], &a[j
		+ j * a_dim1], &sminpr, &sine, &cosine);
	i__2 = j - 1;
	dscal(&i__2, &sine, &work[mn + 1], &c__1);
	work[mn + j] = cosine;
	smin = sminpr;
/* L30: */
    }
...
/*     singular value of A(1:RANK+1,1:RANK+1). */
    ...
    if (*rank < mn) {
	dlaic1(&c__1, rank, &work[1], &smax, &a[(*rank + 1) * a_dim1 + 1], &
		a[*rank + 1 + (*rank + 1) * a_dim1], &smaxpr, &sine, &cosine);
	smax = smaxpr;
	dlaic1(&c__2, rank, &work[mn + 1], &smin, &a[(*rank + 1) * a_dim1 +
		1], &a[*rank + 1 + (*rank + 1) * a_dim1], &sminpr, &sine, &
		cosine);
	dscal(rank, &sine, &work[mn + 1], &c__1);
...
/* Subroutine */ int dglbif(integer *job, integer *m, integer *n, integer *k,
	 doublereal *a, integer *lda, doublereal *c__, integer *ldc, integer *
	jpvt, doublereal *f, integer *rank, logical *permut, doublereal *work,
	 integer *info)
{
    /* System generated locals */
    ...
    static doublereal rdummy[1]	/* was [1][1] */;
...
/*     and store them in vector WORK(RANK:N). */
    ...
    for (j = *rank; j <= i__1; ++j) {
	i__2 = min(*m,j) - *rank + 1;
	work[j] = dnrm2(&i__2, &a[*rank + j * a_dim1], &c__1);
/* L10: */
    }
...
/*     A(RANK:M,RANK:N). Use the data stored in vector WORK(RANK:N). */
    ...
    i__1 = *n - *rank + 1;
    jj = *rank - 1 + idamax(&i__1, &work[*rank], &c__1);

/*     Determine if a permutation must occur. */

    *permut = jj > *rank && (d__1 = work[jj], abs(d__1)) * *f > (d__2 = work[*
	    rank], abs(d__2));
...
/*        to store temporal data. */
	...
	i__1 = min(mn,jj);
	dcopy(&i__1, &a[jj * a_dim1 + 1], &c__1, &work[1], &c__1);
	i__1 = *rank;
	for (j = jj - 1; j >= i__1; --j) {
/* Computing MIN */
	    i__3 = mn, i__4 = j + 1;
	    i__2 = min(i__3,i__4);
	    dcopy(&i__2, &a[j * a_dim1 + 1], &c__1, &a[(j + 1) * a_dim1 + 1],
		     &c__1);
/* L20: */
	}
	i__1 = min(mn,jj);
	dcopy(&i__1, &work[1], &c__1, &a[*rank * a_dim1 + 1], &c__1);
...
/*        Retriangularize matrix A after the permutation. */
	...
	if (*job == 1) {
	    ...
	    i__2 = *n - *rank + 1;
	    dgret(job, &i__1, &i__2, k, &a[*rank + *rank * a_dim1], lda,
...
/* Subroutine */ int dcniif(integer *job, integer *m, integer *n, integer *k,
	 doublereal *a, integer *lda, doublereal *c__, integer *ldc, integer *
	jpvt, doublereal *vnorm, doublereal *f, integer *rank, logical *
	permut, doublereal *work, integer *info)
{
    /* System generated locals */
    integer a_dim1, a_offset, c_dim1, c_offset, i__1, i__2;
    doublereal d__1, d__2;
    ...
    static doublereal sminpr, rdummy[1];
...
/*     Save left singular vector in vector WORK(1:RANK). */
    ...
    smin = (d__1 = a[a_dim1 + 1], abs(d__1));
    work[1] = 1.;
    i__1 = *rank;
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	dlaic1(&c__2, &i__2, &work[1], &smin, &a[j * a_dim1 + 1], &a[j + j *
...
/*        WORK(1:RANK). */
	...
	dtrsv("Upper", "No transpose", "No unit", rank, &a[a_offset], lda, &
		work[1], &c__1);

/*        Find the index with largest absolute value in vector */
/*        WORK(1:RANK). */

	jj = idamax(rank, &work[1], &c__1);

/*        Determine if a permutation must occur. */

	*permut = jj < *rank && (d__1 = work[jj], abs(d__1)) * *f > (d__2 =
		work[*rank], abs(d__2));
...
/*           RANK->RANK-1. Use vector WORK to store temporal data. */
	    ...
	    dcopy(rank, &a[jj * a_dim1 + 1], &c__1, &work[1], &c__1);
	    i__1 = *rank;
	    for (j = jj + 1; j <= i__1; ++j) {
		dcopy(&j, &a[j * a_dim1 + 1], &c__1, &a[(j - 1) * a_dim1 + 1]
			, &c__1);
/* L20: */
	    }
	    dcopy(rank, &work[1], &c__1, &a[*rank * a_dim1 + 1], &c__1);
...
/*           Retriangularize matrix A after the permutation. */
		...
		i__1 = *rank - jj + 1;
		i__2 = *n - jj + 1;
		dhess(job, &i__1, &i__2, k, &a[jj + jj * a_dim1], lda,
			rdummy, &c__1, &work[1], info);
	    } else if (*job == 2) {
		i__1 = *rank - jj + 1;
		i__2 = *n - jj + 1;
		dhess(job, &i__1, &i__2, k, &a[jj + jj * a_dim1], lda, &c__[
...
/* Subroutine */ int dgret(integer *job, integer *m, integer *n, integer *k,
...
/*        Givens Rotations in vector WORK. */
	...
	for (i__ = *m; i__ >= 2; --i__) {
	    dlartg(&a[i__ - 1 + a_dim1], &a[i__ + a_dim1], &work[i__], &work[
		    *m + i__], &r__);
	    a[i__ - 1 + a_dim1] = r__;
	    a[i__ + a_dim1] = 0.;
/* L10: */
...
/* Computing MIN */
		...
		i__3 = j + jb - i__;
		drot(&i__3, &a[i__ - 1 + i__ * a_dim1], lda, &a[i__ + i__ *
...
/* Computing MIN */
		...
		drot(&jb, &a[i__ - 1 + j * a_dim1], lda, &a[i__ + j * a_dim1]
...
/* Computing MIN */
		...
		jb = min(i__5,i__3);
		for (i__ = *m; i__ >= 2; --i__) {
		    drot(&jb, &c__[i__ - 1 + j * c_dim1], ldc, &c__[i__ + j *
...
/*           from the right. */
	    ...
	    for (i__ = *m; i__ >= 2; --i__) {
		drot(k, &c__[(i__ - 1) * c_dim1 + 1], &c__1, &c__[i__ *
...
/*           Compute the rotation parameters and update column 1 of A. */
	    ...
	    dlartg(&a[itemp + a_dim1], &a[i__ + a_dim1], &cosine, &sine, &
		    r__);
	    a[itemp + a_dim1] = r__;
	    a[i__ + a_dim1] = 0.;
...
/*           Update columns I:N of matrix A. */
	    ...
	    i__1 = *n - i__ + 1;
	    drot(&i__1, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ * a_dim1]
		    , lda, &cosine, &sine);
...
/*              Apply the previously computed rotations from the left. */
		...
		drot(k, &c__[itemp + c_dim1], ldc, &c__[i__ + c_dim1], ldc, &
			cosine, &sine);
	    } else if (*job == 3 && *k > 0) {
...
/*              from the right. */
		...
		drot(k, &c__[itemp * c_dim1 + 1], &c__1, &c__[i__ * c_dim1 +
			1], &c__1, &cosine, &sine);
	    }
/* L90: */
	}
...
/* Subroutine */ int dhess(integer *job, integer *m, integer *n, integer *k,
	doublereal *a, integer *lda, doublereal *c__, integer *ldc,
	doublereal *work, integer *info)
{
    /* System generated locals */
...
/* Computing MIN */
		...
		itemp = i__ - 1;
		dlartg(&a[itemp + itemp * a_dim1], &a[i__ + itemp * a_dim1],
			&work[i__], &work[*m + i__], &r__);
		a[itemp + itemp * a_dim1] = r__;
		a[i__ + itemp * a_dim1] = 0.;
		i__4 = j + jb - i__;
		drot(&i__4, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ *
			a_dim1], lda, &work[i__], &work[*m + i__]);
/* L30: */
	    }
/* L10: */
...
/*           Update columns I-1:N of matrix A. */
	    ...
	    a[i__ + itemp * a_dim1] = 0.;
	    i__2 = *n - i__ + 1;
	    drot(&i__2, &a[itemp + i__ * a_dim1], lda, &a[i__ + i__ * a_dim1]
		    , lda, &cosine, &sine);
