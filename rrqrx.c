 ...
 * [Q,R,p,r] = rrqrx(A,0)
 * [Q,R,p,r] = rrqrx(A,job)
 * [Q,R,p,r] = rrqrx(A,job,0)
 *
 * compile command:
 * mex -O rrqrx.c libmwlapack.lib rrqr.lib
 * or
...
void rrqrx_double(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (cplx) {
        zgeqpx(&job, &m, &n, &k, Ap, &m, Cp, &ldc, Pp, &ircond, &orcond, &rank, slvalues, pwork, &lwork, rwork, &info);
    }
    else {
        dgeqpx(&job, &m, &n, &k, Ap, &m, Cp, &ldc, Pp, &ircond, &orcond, &rank, slvalues, pwork, &lwork, &info);
    }
    mxFree(pwork);
    mxFree(slvalues);
    if (cplx) {
    ...
    if (info != 0) {
        ...
        if (info == 1) {
            ...
            if (cplx) {
                mexErrMsgTxt("ZGEQPX not succesful");
            }
            else {
                mexErrMsgTxt("DGEQPX not succesful");
            }
        }
    }
...
{   
    ...
    /* calls the DGEQPY function */
    if (cplx) {
        cgeqpx(&job, &m, &n, &k, Ap, &m, Cp, &ldc, Pp, &ircond, &orcond, &rank, slvalues, pwork, &lwork, rwork, &info);
    }
    else {
        sgeqpx(&job, &m, &n, &k, Ap, &m, Cp, &ldc, Pp, &ircond, &orcond, &rank, slvalues, pwork, &lwork, &info);
    }
    mxFree(pwork);
    mxFree(slvalues);
    if (cplx) {
    ...
    if (info != 0) {
        ...
        if (info == 1) {
            ...
            if (cplx) {
                mexErrMsgTxt("CGEQPX not succesful");
            }
            else {
                mexErrMsgTxt("SGEQPX not succesful");
            }
        }
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1 && nrhs != 2 && nrhs != 3) {
        mexErrMsgTxt("RRQRX requires one, two or three arguments.");
    }
    if (nlhs > 4) {
    ...
    if (mxIsDouble(prhs[0])) {
        rrqrx_double(nlhs, plhs, nrhs, prhs);
    }
    else if (mxIsSingle(prhs[0])) {
        rrqrx_single(nlhs, plhs, nrhs, prhs);
    }
    else {
